...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        # An explicit per-request model override (OpenAI-compatible ``model``
        # field) wins over both the config model and any fallback model, and
        # is normalized + api_mode-derived for the active provider (Copilot
        # GPT-5.x needs Responses even when the config default is a
        # chat-completions model). Fork §11.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                ...
                from agent.display import build_tool_preview, get_tool_display_label, get_tool_emoji
                label = build_tool_preview(function_name, function_args) or function_name
                _stream_q.put(("__tool_progress__", {
                    "tool": function_name,
                    "emoji": get_tool_emoji(function_name),
                    "label": label,
                    "displayLabel": get_tool_display_label(function_name, function_args),
