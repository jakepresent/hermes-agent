...
class TestBuildToolPreview:
    ...
    def test_terminal_preview_skips_leading_strict_mode_line(self):
        # §5 intent: the `set -euo pipefail` boilerplate is stripped and the
        # meaningful command is shown. Exact compaction now comes from the
        # upstream shell summarizer (which also collapses compound commands).
        command = "set -euo pipefail\nprintf 'node: '; node --version\nnpm test"
        assert build_tool_preview("terminal", {"command": command}) == "printf 'node: ' + 1 command"
    ...
    def test_terminal_preview_skips_inline_strict_mode_prefix(self):
        # §5 intent preserved: leading `set -euo pipefail;` is stripped. The
        # remaining compound command is compacted by the upstream summarizer.
        command = "set -euo pipefail; npm install && npm test"
        assert build_terminal_command_preview(command) == "npm install + 1 command"
