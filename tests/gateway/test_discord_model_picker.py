...
def test_model_picker_provider_view_shows_current_model():
    """The picker must state the CURRENT model before offering a switch.

    Upstream converged on this behavior but renamed the seam: the fork built the
    text in ``_build_provider_content()``, upstream composes it in the embed
    inside ``send_model_picker`` and re-renders it on back-navigation. Asserting
    the behavior instead of the helper keeps the gate meaningful across the
    rename.
    """
    view = ModelPickerView(
        providers=[
            {
                # upstream schema: slug/name (the fork used provider/label)
                "slug": "copilot",
                "name": "GitHub Copilot",
                "models": ["gpt-5.5"],
                "total_models": 1,
    ...
    # The view carries the current selection...
    assert view.current_model == "gpt-5.5"
    assert view.current_provider == "copilot"

    # ...and offers the provider dropdown as the first step.
    select = next(
        child
        for child in view.children
        if getattr(child, "custom_id", None) == "model_provider_select"
    )
    assert any(opt.value == "copilot" for opt in select.options)
