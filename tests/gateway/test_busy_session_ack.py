...
class TestBusySessionAck:
    ...
    async def test_steer_mode_calls_agent_steer_no_interrupt_no_queue(self, monkeypatch):
        ...
        # Upstream wraps the payload via _steer_text_with_origin() so the injection
        # carries routing origin (and honors privacy.redact_pii) instead of bare
        # text — assert the text is CARRIED, not that the call is byte-identical.
        agent.steer.assert_called_once()
        steered_text = agent.steer.call_args[0][0]
        assert "also check the tests" in steered_text
        agent.interrupt.assert_not_called()
    ...
    async def test_steer_mode_transcribes_voice_before_injection(self, monkeypatch):
        ...
        # Upstream wraps the payload with routing origin; assert the TRANSCRIPT
        # reached the agent rather than pinning the exact call signature.
        agent.steer.assert_called_once()
        assert "yönü teknik mimariye çevir" in agent.steer.call_args[0][0]
        agent.interrupt.assert_not_called()
