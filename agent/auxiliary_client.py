...
def _client_cache_key(
    ...
    *,
    ...
    runtime_key = tuple(
        ...
        for field in _MAIN_RUNTIME_FIELDS
    ) if normalized_provider == "auto" else ()
    # `auto` can now resolve through task-specific or main fallback policy,
    # so the task participates in the cache key. Non-auto providers keep the
    # old cache shape because the explicit provider/model tuple is sufficient.
    task_key = (task or "") if normalized_provider == "auto" else ""
    ...
    return (normalized_provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
