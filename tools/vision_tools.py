...
def _detect_image_mime_type(image_path: Path) -> Optional[str]:
    """Return a MIME type when a local file contains image bytes.

    Keep the path wrapper used by the fork's OCR helpers, but delegate raster
    magic-byte detection to the upstream byte-level chokepoint.
    """
    try:
        ...
        return None
    detected = _detect_image_mime_type_from_bytes(header)
    if detected:
        return detected
