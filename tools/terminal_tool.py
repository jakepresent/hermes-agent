...
Do NOT pipe command output directly into interpreters (`cmd | python`, `cmd | node`, `cmd | bash`, etc.). Hermes' security scanner treats pipe-to-interpreter as risky. Save output to a temp file and pass its path to the interpreter, or use execute_code/hermes_tools for multi-step parsing.
NEVER pipe a build/test command through tail/head/cat to shorten output: auto-truncation preserves the full output, while a pipe can mask the real command's exit status. Run it bare; likewise avoid `cmd || echo failed`, which masks failure.
