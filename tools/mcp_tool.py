...
class MCPServerTask:
    ...
    async def _initialize_and_discover(self, session: Any, config: dict) -> None:
        ...
        self._ready.set()
        # Session is live again: clear any breaker state from a prior outage so
        # the first call after recovery isn't gated on a stale
        # consecutive-failure count (#16788).
    ...
    async def _wait_for_reconnect_or_standby(self, standby_interval: float) -> str:
        """Park until reconnect/shutdown OR a standby timer elapses (fork §19).

        Same dormant-listener semantics as
        :meth:`_wait_for_reconnect_or_shutdown` (so ``_reconnect_event`` always
        has a listener and the server is never orphaned), but ALSO wakes on a
            ...
            try:
                ...
                if retries > _MAX_RECONNECT_RETRIES:
                    ...
                    logger.warning(
                        ...
                        "parking as a dormant listener and auto-retrying every "
                        "%ds until the endpoint recovers: %s",
                    ...
                    # (upstream). A future _reconnect_event.set() — from the
                    # breaker's half-open probe, OAuth recovery, or a manual
                    # /mcp refresh — wakes us to rebuild the transport. On top
                    # of that (fork §19), a standby timer ALSO wakes us on its
                    # own so long-lived Agency bridges self-heal with no manual
                    # action once the endpoint comes back. Because
                    # _run_http/_run_stdio re-discover tools on every successful
                    # (re)entry, standby reconnect restores the model-facing
                    # tool names with no /reload-mcp and no prompt-cache churn.
                    # _RECONNECT_STANDBY_INTERVAL == 0 restores the old
                    # give-up/park-forever behavior (no timer wake).
                    self._deregister_tools()
                    self._reconnect_event.clear()
                    if _RECONNECT_STANDBY_INTERVAL > 0:
                        parked = await self._wait_for_reconnect_or_standby(
                            _RECONNECT_STANDBY_INTERVAL
                        )
                    else:
                    ...
                    if parked == "shutdown":
                        return
                    logger.info(
                        "MCP server '%s': %s; rebuilding transport.",
                        self.name,
                        "standby retry" if parked == "standby" else "reconnect requested while parked",
                    )
