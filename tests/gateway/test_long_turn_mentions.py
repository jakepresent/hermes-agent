...
def test_long_turn_mention_combined_elapsed_and_tool_rule_requires_both():
    cfg = _config([
        {"elapsed_seconds": 90},
        {"elapsed_seconds": 45, "tool_calls": 6},
    ])
    source = _discord_source()

    assert not _long_turn_mention_text_for_source(
        source, cfg, "discord", elapsed_seconds=44.9, tool_calls=6, surface="final"
    )
    assert not _long_turn_mention_text_for_source(
        source, cfg, "discord", elapsed_seconds=45, tool_calls=5, surface="final"
    )
    assert _long_turn_mention_text_for_source(
        source, cfg, "discord", elapsed_seconds=45, tool_calls=6, surface="final"
    ) == "<@123456789>"
    assert _long_turn_mention_text_for_source(
        source, cfg, "discord", elapsed_seconds=90, tool_calls=0, surface="final"
    ) == "<@123456789>"
...
def test_long_turn_mention_surface_gates_final_vs_approval():
    ...
    assert _long_turn_mention_text_for_source(
        source, cfg, "discord", elapsed_seconds=45, tool_calls=6, surface="approval"
    ) == "<@123456789>"
