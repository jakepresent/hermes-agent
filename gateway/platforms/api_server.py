...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        # An explicit per-request model override wins when it is not a route
        # alias and no session-level /model override already owns the session.
        if (
            not route
            and not session_override
            and model_override
            and str(model_override).strip()
            and str(model_override).strip() != self._model_name
        ):
