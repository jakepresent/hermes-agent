...
# --- Fork gate: /model status must never mutate config ------------------------


def test_status_aliases_are_read_only_by_construction():
    """``/model status`` and friends must not reach the switch path.

    Upstream's parser treats every bare word as a model NAME, so without this
    guard ``/model status`` attempts to switch to a model called "status" and
    ``/model status --global`` PERSISTS it to config.yaml. Verified directly
    against the parser so the test fails loudly if upstream ever adds real
    parsing for these words.
    """
    from gateway.slash_commands_model import _MODEL_STATUS_ALIASES
    from hermes_cli.model_switch import parse_model_switch_args

    for alias in ("status", "current", "show", "info"):
        assert alias in _MODEL_STATUS_ALIASES
        # the parser itself still yields it as a switch target...
        assert parse_model_switch_args(alias).target == alias
        # ...which is exactly why the gateway must short-circuit first.


def test_status_alias_matching_is_case_and_flag_insensitive():
    """``/model STATUS --global`` must still be read-only."""
    from gateway.slash_commands_model import _MODEL_STATUS_ALIASES
    from hermes_cli.model_switch import parse_model_switch_args

    request = parse_model_switch_args("STATUS --global")
    assert request.is_global is True  # the dangerous flag IS parsed
    assert str(request.target).strip().lower() in _MODEL_STATUS_ALIASES
