...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "where Z\", or an `@session:<profile>/<id>` link. Source-first limit: "
        "session_search covers Hermes conversation history only. If the user "
        "provided a direct source identifier, inspect that direct source first "
        "when accessible, and use session_search as secondary historical context "
        "(or when the source is not found / inaccessible). For durable project "
