...
@pytest.mark.asyncio
async def test_new_session_exposes_edit_approvals_as_modes_and_thought_as_config(agent):
    resp = await agent.new_session(cwd="/tmp")

    assert [option.id for option in resp.config_options] == ["thought_level"]
...
async def test_set_config_option_persists_edit_approval_policy_without_advertising_config(agent):
    ...
    assert [option.id for option in update.config_options] == ["thought_level"]
...
class TestRegisterSessionMcpServers:
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        # get_tool_definitions owns provider-tool injection. ACP adopts that
        # canonical snapshot rather than appending memory-manager schemas again.
