...
discord:
  ...
  # Optional: independently verify the Discord REST delivery path.
  rest_liveness_interval_seconds: 60
  rest_liveness_failure_threshold: 3
```

The WebSocket and REST probes use separate task handles and failure counters. Either persistent failure enters the same bounded adapter-reconnect path; successful REST checks never reset or mask stale WebSocket ACK state. Set either interval or threshold to `0` to disable that specific probe.

The old `liveness_interval_seconds` and `liveness_failure_threshold` names remain compatibility aliases for the WebSocket probe only; they no longer mean REST probing.
