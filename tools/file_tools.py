...
SEARCH_FILES_SCHEMA = {
    "name": "search_files",
    "description": "Search the live filesystem: file contents or file names in the current workspace. Use this for source code, repo files, generated outputs, and current on-disk state — not for durable project memory or prior conversation recall (use memory_search/session_search for those). Use this instead of grep/rg/find/ls in terminal. Ripgrep-backed when available. On macOS, broad searches above the user home automatically skip TCC-protected folders (Desktop, Documents, Downloads, Library, Movies, Music, Pictures); target one directly when access is intentional.\n\nContent search (target='content'): Regex search inside files. Output modes: full matches with line numbers, file paths only, or match counts.\n\nFile search (target='files'): Find files by glob pattern (e.g., '*.py', '*config*'). Also use this instead of ls — results are sorted by modification time.",
    "parameters": {
        "type": "object",
        "properties": {
