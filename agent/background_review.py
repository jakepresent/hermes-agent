...
_MEMORY_REVIEW_PROMPT = (
    "Review the conversation above conservatively. Save only a genuinely new, "
    "durable fact about the user: a stable preference, personal detail, correction, "
    "or expectation that will matter in future sessions.\n\n"
    "Before writing, compare the candidate with memory already present in the "
    "conversation. Do not duplicate or paraphrase a fact that is already present; "
    "replace a stale entry when the new fact supersedes it. Do not save project "
    "status, task progress, temporary todos, completed-work logs, raw evidence, "
    "review metadata, memory-usage bookkeeping, or failed-write details. Those "
    "belong in canonical project/domain files or session history, not injected "
    "memory.\n\n"
    "Doing nothing is the normal outcome. If there is no genuinely new durable "
    "user fact, say 'Nothing to save.' and stop."
)

_SKILL_REVIEW_PROMPT = (
    "Review the conversation above conservatively for reusable procedural "
    "learning. Doing nothing is the normal outcome. Update the skill library "
    "only when the session produced a verified reusable workflow, a user "
    "correction that changes how this class of task should be handled, or proof "
    "that an existing skill is wrong or missing a required step.\n\n"
    ...
    "skill covers the class. Use skills_list first and reject a near-duplicate "
    "name or trigger. Keep the description self-contained and at most 160 characters. "
    ...
    "Doing nothing is the normal outcome. If the session ran smoothly with no "
    "correction, verified reusable technique, or broken skill, say 'Nothing to "
    "save.' and stop."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and update two things:\n\n"
    "**Memory**: save only a genuinely new durable user fact. Compare it with "
    "memory already present; do not duplicate or paraphrase an existing fact. "
    "Do not save project status, temporary todos, completed-work logs, raw "
    "evidence, review metadata, memory-usage bookkeeping, or failed-write "
    "details. Replace a stale entry when the new fact supersedes it.\n\n"
    "**Skills**: update only for a verified reusable workflow, a user correction "
    "that changes this class of task, or proof that an existing skill is wrong "
    "or missing a required step. Doing nothing is the normal outcome.\n\n"
    ...
    "Use skills_list first and reject a near-duplicate name or trigger. Keep "
    "the description self-contained and at most 160 characters. Name at the "
    "class level — NOT a PR number, error string, "
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act only on a dimension with verified durable signal. If neither qualifies, "
