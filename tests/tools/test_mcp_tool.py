    ...
    def test_initial_oauth_failure_does_not_retry(self):
        ...
        async def _test():
            ...
            with patch.object(MCPServerTask, "_run_stdio", patched_run_stdio), \
                ...
                # Permanent auth failures park immediately without retrying, but
                # keep the lifecycle task alive so later credential refresh can
                # revive the server. Shut it down explicitly after observing the
                # parked state.
                assert run_count == 1
