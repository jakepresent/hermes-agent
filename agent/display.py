...
def build_terminal_command_preview(
    ...
    (fork §5).  For plain shell commands, defers to ``summarize_shell_command``
    (upstream) so plumbing tails (``| tail -20``) and multi-command probes are
    ...
    # No heredoc body — hand the strict-mode-stripped command to the upstream
    # shell summarizer for plumbing/multi-command compaction.
    summarized = summarize_shell_command(text)
    if summarized:
        return _truncate_preview(summarized, max_len)
...
def build_tool_preview(tool_name: str, args: dict, max_len: int | None = None) -> str | None:
    ...
    *max_len* controls truncation.  ``None`` (default) defers to the global
    ...
    if tool_name in {"terminal", "execute_code"}:
        ...
        return build_terminal_command_preview(args.get(key), max_len=max_len)
...
def _get_cute_tool_message(
    ...
    if tool_name == "terminal":
        ...
        return _wrap(f"┊ 💻 $         {_trunc(command_preview or args.get('command', ''), 42)}  {dur}")
    if tool_name == "process":
        action = args.get("action", "?")
