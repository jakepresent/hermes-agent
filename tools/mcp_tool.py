...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_recover_now_event", "_reconnect_retries",
        "_session_proven", "_was_parked",
    )
    ...
    async def _initialize_and_discover(self, session: Any, config: dict) -> None:
        ...
        # A completed handshake is not proof of a durable session. Preserve
        # upstream's rapid-drop budget until keepalive or a tool call proves it.
    ...
    async def _wait_for_reconnect_or_shutdown(
        ...
        recovery_task = asyncio.ensure_future(self._recover_now_event.wait())
        try:
            await asyncio.wait(
                {shutdown_task, reconnect_task, recovery_task},
                return_when=asyncio.FIRST_COMPLETED,
                timeout=timeout,
            )
        finally:
            for t in (shutdown_task, reconnect_task, recovery_task):
                if not t.done():
                    t.cancel()
                    try:
                        await t
        ...
        while True:
            try:
                ...
                if not self._ready.is_set():
                    ...
                    await _sleep_or_recovery(_jittered(backoff))
